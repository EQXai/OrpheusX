...
# This helper checks the component class to ensure compatibility.

def info_kwargs(comp_cls: type, text: str) -> dict:
    """Return a dictionary with "info" if supported by the component."""
...
with gr.Blocks(css=GLOBAL_CSS) as demo:
    ...
    with gr.Tab("Train LoRA"):
        ...
        hf_input = gr.Textbox(
            ...
            **info_kwargs(gr.Textbox, "Hugging Face dataset URLs"),
        )
        local_ds = gr.Dropdown(
            choices=dataset_choices,
            multiselect=True,
            label="Local dataset(s)",
            **info_kwargs(gr.Dropdown, "Datasets prepared locally"),
        )
