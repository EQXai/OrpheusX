...
import gc
import time
from tools.logger_utils import get_logger, UILogHandler
...
def generate_long_form(
    ...
    try:
        ...
        for idx, seg in enumerate(segments, 1):
            ...
            torch.cuda.empty_cache()
            gc.collect()
            progress(idx / len(segments), desc=f"Chunk {idx}/{len(segments)}")
            yield ui_handler.pop(), None

        if final_audio is None:
            yield ui_handler.pop() or "Generation failed", None
            return
